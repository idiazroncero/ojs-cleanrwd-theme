...
{/if}
	...
	<form method="post" id="submit" action="{url op="subscriptions" path="individual"}" class="form-row">
...
<table class="listing">
	<thead>
		<th>{translate key="manager.subscriptions.user"}</th>
...
{iterate from=subscriptions item=subscription}
	...
	<tr >
		<td data-title='{translate key="manager.subscriptions.user"}'>
