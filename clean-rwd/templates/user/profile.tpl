...
<div class="form-row">
	<p class="label">{fieldLabel name="gender" key="user.gender"}</p>
...
<div class="form-row">
	<p class="label">{fieldLabel name="affiliation" key="user.affiliation"}</p>
...
{if $currentJournal}
	<div class="form-row">
		<p class="label">{translate key="user.roles"}</p>
			{if $allowRegReader}
				<input type="checkbox" id="readerRole" name="readerRole" {if $isReader || $readerRole}checked="checked" {/if}/>&nbsp;{fieldLabel name="readerRole" key="user.role.reader"}
			{/if}
			{if $allowRegAuthor}
				<input type="checkbox" id="authorRole" name="authorRole" {if $isAuthor || $authorRole}checked="checked" {/if}/>&nbsp;{fieldLabel name="authorRole" key="user.role.author"}
			{/if}
			{if $allowRegReviewer}
				<input type="checkbox" id="reviewerRole" name="reviewerRole" {if $isReviewer || $reviewerRole}checked="checked" {/if}/>&nbsp;{fieldLabel name="reviewerRole" key="user.role.reviewer"}
			{/if}
	</div>
{/if}
{if $allowRegReviewer || $isReviewer}
<div class="form-row">
	<p class="label">{fieldLabel name="interests" key="user.interests"}</p>
		{include file="form/interestsInput.tpl" FBV_interestsKeywords=$interestsKeywords FBV_interestsTextOnly=$interestsTextOnly}
</div>
{/if}

<div class="form-row">
	<p class="label">{fieldLabel name="biography" key="user.biography"}{translate key="user.biography.description"}</p>
...
<div class="form-row">
	...
	{foreach from=$availableLocales key=localeKey item=localeName}
		<input type="checkbox" name="userLocales[]" id="userLocales-{$localeKey|escape}" value="{$localeKey|escape}"{if in_array($localeKey, $userLocales)} checked="checked"{/if} /> <label for="userLocales-{$localeKey|escape}">{$localeName|escape}</label>
	{/foreach}
	</div>
</div>
{/if}
...
{if $displayOpenAccessNotification}
	...
	{foreach from=$journals name=journalOpenAccessNotifications key=thisJournalId item=thisJournal}
		...
		{if !$notFirstJournal}
			...
			<div class="form-row">
				<p class="label">{translate key="user.profile.form.openAccessNotifications"}
		{/if}

		{if $publishingMode == $smarty.const.PUBLISHING_MODE_SUBSCRIPTION && $enableOpenAccessNotification}
			<input type="checkbox" name="openAccessNotify[]" {if $notificationEnabled}checked="checked" {/if}id="openAccessNotify-{$thisJournalId|escape}" value="{$thisJournalId|escape}" /> <label for="openAccessNotify-{$thisJournalId|escape}">{$thisJournal->getLocalizedTitle()|escape}</label>
		{/if}
...
<div class="buttons">
	...
	<input type="button" value="{translate key="common.cancel"}" class="button" onclick="document.location.href='{url page="user"}'" />
</div>
