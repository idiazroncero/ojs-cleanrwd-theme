...
<table class="listing" id="reviewFormElementsTable">
	<thead>
		<th>&nbsp;</th>
		<th>{translate key="manager.reviewFormElements.question"}</th>
...
{if $reviewFormElements->wasEmpty()}
	<tr>
		<td colspan="3">{translate key="manager.reviewFormElements.noneCreated"}</td>
	</tr>
{else}
	<tr class="listing-pager">
		<td colspan="2">{page_info iterator=$reviewFormElements}</td>
		<td>{page_links anchor="reviewFormElements" name="reviewFormElements" iterator=$reviewFormElements}</td>
	</tr>
{/if}
</tbody>
...
<div class="buttons">
	<a class="button" href="{url op="createReviewFormElement" path=$reviewFormId}">{translate key="manager.reviewFormElements.create"}</a>
</div>
</div>
